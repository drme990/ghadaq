# Ghadaq Association — Public Web App

A bilingual (Arabic / English) **Next.js 16** public-facing application for **Ghadaq Association**, offering Islamic charitable services including Aqiqah, sacrifices, vows, and charity. Built with a custom gold/green theme and full RTL support.
...
```
User → ghadaq (:3002) → /api/* rewrite → next-backend (:3000) → MongoDB Atlas
```

Orders originating from this app are tagged `source: 'ghadaq'` (prefix `GHD-`). Users landing on a GHD- order status page at the Manasik URL are automatically redirected to the Ghadaq URL.
...
- Primary: **#ffc001** (gold / amber)
- Gradient: `#ffa401 → #ffc001 → #ffd84d`
- Dark mode: forest green `#134d37`
...
| `manasik-v2/`   | Sister public app for Manasik Foundation |
