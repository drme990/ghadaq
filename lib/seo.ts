...
export function getSeoMetadata({
  ...
  const baseUrl = (process.env.BASE_URL || 'https://www.ghadaqplus.com').replace(/\/$/, '');
  ...
  return {
    ...
    // This prevents double-branding and prevents the layout's title.default
    // from rendering as a separate <title> tag alongside the page's title.
...
export function buildOrganizationSchema(locale: string, baseUrl: string) {
  ...
  const name = isAr ? 'مؤسسة غدق' : 'Ghadaq Plus';
  const description = isAr
    ? 'مؤسسة غدق - نُؤدي عنك بالوكالة الشرعية أداء العمرة، العقيقة، الأضاحي، النذر، الصدقة، وحفر الآبار.'
    : 'Ghadaq Plus performs religious services on your behalf with legal proxy: Umrah Badal, Aqiqah, Qurbani, Sadaqah, and water wells.';

  return {
    '@context': SCHEMA_CONTEXT,
    '@type': 'Organization',
    name,
    alternateName: 'Ghadaq Plus',
    url: baseUrl,
    logo: `${baseUrl}/logo-light.png`,
    description,
    sameAs: [
      'https://www.facebook.com/ghadaq990',
      'https://www.tiktok.com/@ghadaq990',
      'https://www.instagram.com/ghadaq990',
    ],
    contactPoint: {
      '@type': 'ContactPoint',
...
export function buildWebsiteSchema(locale: string, baseUrl: string) {
  ...
  return {
    '@context': SCHEMA_CONTEXT,
    '@type': 'WebSite',
    name: isAr ? 'مؤسسة غدق' : 'Ghadaq Plus',
    url: baseUrl,
