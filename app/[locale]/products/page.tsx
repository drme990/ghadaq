...
export async function generateMetadata({
  ...
  const brandName = isAr ? 'مؤسسة غدق' : 'Ghadaq Plus';
...
async function getProductsForSeo(): Promise<Product[]> {
  try {
    ...
    const params = new URLSearchParams({ platform: 'ghadaq' });
...
export default async function ProductsPage() {
  ...
  const productsJsonLd = {
    ...
    '@type': 'ItemList',
    name: isAr ? 'منتجات مُؤسسة غدق' : 'Ghadaq Plus Services',
    description: isAr
      ? 'تصفح جميع خدمات مؤسسة غدق: عمرة البدل، العقيقة، الأضاحي، النذر، الصدقة، وحفر الآبار.'
      : 'Browse all Ghadaq Plus services: Umrah Badal, Aqiqah, Qurbani, Sadaqah, and water wells.',
  ...
  return (
    <>
      ...
      <main className="grid-bg min-h-screen">
        ...
        <Container>
          <ProductsListClient platform="ghadaq" locale={locale} />
        </Container>
      </main>
      <Footer />
      <GoToTop />
