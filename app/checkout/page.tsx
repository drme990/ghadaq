...
async function fetchUpgradeProduct(
  ...
  try {
    const res = await fetch(
      `/api/products/${encodeURIComponent(upgradeRef)}?platform=ghadaq`,
    );
...
async function fetchRecommendProduct(
  ...
  try {
    const res = await fetch(
      `/api/products/${encodeURIComponent(recommendRef)}?platform=ghadaq`,
    );
    const data = await res.json();
    if (!data.success) return null;
    return data.data as Product;
...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    const loadCurrentUserBilling = async () => {
      try {
        const response = await fetch('/api/auth/ghadaq/session', {
          cache: 'no-store',
        });
        if (!response.ok) {
          setIsAuthenticatedCheckout(false);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        const res = await fetch(
          `/api/products/${activeProductSlug}?platform=ghadaq`,
        );
        const data = await res.json();
        if (data.success) {
  ...
  const ensureCheckoutAccountAuthentication = async (): Promise<boolean> => {
    ...
    try {
      const registerRes = await fetch('/api/auth/ghadaq/register', {
        method: 'POST',
      ...
      const loginRes = await fetch('/api/auth/ghadaq/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: email.trim(),
