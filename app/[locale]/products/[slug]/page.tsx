...
export async function generateMetadata({
  ...
  const viewerCountryCode = await getViewerCountryCode('ghadaq-home-country');
  const product = await getProduct(slug, viewerCountryCode);

  if (!product) {
    return {
      title: { absolute: locale === 'ar' ? 'المنتج غير موجود | مؤسسة غدق' : 'Product Not Found | Ghadaq Plus' },
    };
  }
    ...
    <>
      ...
      <main className="grid-bg min-h-dvh">
        <Container>
          ...
          <ProductDetailsClient product={product} platform="ghadaq" />
        </Container>
        <Testimonials />
        <Container>
          <FAQDisplay />
