...
function syncClientAuthCookie(
  ...
  if (hasToken && !hasClientAuthCookie) {
    response.cookies.set('ghadaq-auth', '1', {
      httpOnly: false,
      secure: isProduction,
  ...
  if (!hasToken && hasClientAuthCookie) {
    response.cookies.set('ghadaq-auth', '', {
      httpOnly: false,
      secure: isProduction,
      sameSite: isProduction ? 'none' : 'lax',
      maxAge: 0,
...
export function proxy(request: NextRequest) {
  ...
  const hasSession = Boolean(request.cookies.get('ghadaq-token')?.value);
  const hasClientAuthCookie = Boolean(
    request.cookies.get('ghadaq-auth')?.value,
  );
